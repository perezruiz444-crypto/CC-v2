...
export function useObligaciones(empresaId: string | null): UseObligacionesResult {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const normalized = (data ?? []).map((o: any) => {
        const catalogo = o.catalogo_id as CatalogoData
        return {
          ...o,
          catalogo: catalogo ?? { id: '', nombre: 'Desconocido', descripcion: null, categoria: '', periodicidad: null, fundamento_legal: null, notas_importantes: null, multa_minima_mxn: null, multa_maxima_mxn: null },
  ...
  const crearObligacionPersonalizada = useCallback(async (
    ...
    if (projErr) {
      if (import.meta.env.DEV) console.warn(`Advertencia al proyectar vencimientos: ${projErr.message}`)
